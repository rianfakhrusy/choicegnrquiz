 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_choicegnrquiz_edit_form extends question_edit_form {
    /**
    ...
    protected function definition_inner($mform) {
        ...
        $menu = array(
            get_string('answersingleno', 'qtype_choicegnrquiz'),
            get_string('answersingleyes', 'qtype_choicegnrquiz'),
        );
        $mform->addElement('select', 'single',
                get_string('answerhowmany', 'qtype_choicegnrquiz'), $menu);
        $mform->setDefault('single', 1);

        $mform->addElement('advcheckbox', 'shuffleanswers',
                get_string('shuffleanswers', 'qtype_choicegnrquiz'), null, null, array(0, 1));
        $mform->addHelpButton('shuffleanswers', 'shuffleanswers', 'qtype_choicegnrquiz');
        $mform->setDefault('shuffleanswers', 1);

        $mform->addElement('select', 'answernumbering',
                get_string('answernumbering', 'qtype_choicegnrquiz'),
                qtype_choicegnrquiz::get_numbering_styles());
        $mform->setDefault('answernumbering', 'abc');

        $this->add_per_answer_fields($mform, get_string('choiceno', 'qtype_choicegnrquiz', '{no}'),
    ...
    public function validation($data, $files) {
        ...
        foreach ($answers as $key => $answer) {
            ...
            if ($trimmedanswer === '') {
                $errors['fraction['.$key.']'] = get_string('errgradesetanswerblank', 'qtype_choicegnrquiz');
            }
        ...
        if ($answercount == 0) {
            $errors['answer[0]'] = get_string('notenoughanswers', 'qtype_choicegnrquiz', 2);
            $errors['answer[1]'] = get_string('notenoughanswers', 'qtype_choicegnrquiz', 2);
        ...
        if ($data['single']) {
            if ($maxfraction != 1) {
                $errors['fraction[0]'] = get_string('errfractionsnomax', 'qtype_choicegnrquiz',
                        $maxfraction * 100);
            }
        } else {
            $totalfraction = round($totalfraction, 2);
            if ($totalfraction != 1) {
                $errors['fraction[0]'] = get_string('errfractionsaddwrong', 'qtype_choicegnrquiz',
                        $totalfraction * 100);
            }
        }
